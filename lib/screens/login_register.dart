...
class LoginRegisterScreen extends StatefulWidget {
  final login;
  final first;
  const LoginRegisterScreen({Key key, this.login = false, this.first = false})
      : super(key: key);
  @override
...
class _LoginRegisterScreenState extends State<LoginRegisterScreen> {
  ...
  Widget decoratedTextField(String hint, double fontSize,
      TextEditingController controller, BuildContext context,
      {bool obscure = false,
      TextAlign align = TextAlign.center,
      TextInputType type = null,
      int length = null,
      List<TextInputFormatter> list = null}) {
    ...
    return Container(
      ...
      child: TextField(
          ...
          decoration: InputDecoration(
              ...
              counterText: '',
              hintStyle: TextStyle(
                  fontSize: 15, color: cHint, fontFamily: fntAljazeera),
    ...
    if (widget.login)
      try {
        if (_pwd.text.trim() == '' || _email.text.trim() == '')
          ...
          var response = await db.loginData(
            _email.text,
            _pwd.text,
            true,
          ); //lalla@seba_123.com
      ...
      try {
        if (_pwd.text != _cPwd.text || _pwd.text.length < 8)
          ...
          var response = await db.registerData(
              _email.text, _pwd.text, _cPwd.text); //lalla@seba_123.com
          print('response register: ${response}');
          if (response)
            Navigator.of(context).push(MaterialPageRoute(
                builder: (context) => CheckPage(email: _email.text)));
          else {
            if (msgError.trim() != '') {
              setState(() {
                msgShow = msgError;
            ...
            setState(() {
              if (db.rsponseMsg != null) {
                msgError =
                    db.rsponseMsg.replaceAll('{', '').replaceAll('[', '');
                msgError = msgError.replaceAll('}', '').replaceAll(']', '');
              }
            });
            ...
            if (msgError.toLowerCase() == err_emailTaken.toLowerCase())
              Navigator.of(context).push(MaterialPageRoute(
                  builder: (BuildContext context) =>
                      LoginRegisterScreen(login: true)));
  ...
  Widget build(BuildContext context) {
    ...
    var brBtn =
        MediaQuery.of(context).size.height * (widget.login ? 0.035 : 0.03);
    var afBtn =
        MediaQuery.of(context).size.height * (widget.login ? 0.04 : 0.02);
              ...
              child: Stack(
                children: <Widget>[
                      ...
                      children: <Widget>[
                        ...
                        Container(
                            margin: EdgeInsetsDirectional.only(
                                top: vide, bottom: brTitle),
                            width:
                                logoSize, // MediaQuery.of(context).size.width*0.45,
                            height:
                                logoSize, //MediaQuery.of(context).size.width*0.45,
                            child: Image.asset(
                        ...
                        Container(
                          child: Opacity(
                            opacity: 0.78,
                            child: Text(
                              widget.login ? str_login : str_register,
                              style: TextStyle(
                                fontSize: 22,
                        ...
                        decoratedTextField(str_email, 15, _email, context,
                            type: TextInputType.emailAddress),
                        decoratedTextField(str_password, 15, _pwd, context,
                            obscure: true),
                        !widget.login
                            ? decoratedTextField(
                                str_cPassword, 15, _cPwd, context,
                                obscure: true)
                            : Text(
                                '',
                                style: TextStyle(
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            image: DecorationImage(
                                image: AssetImage(!btnPressed
                                    ? 'assets/images/btnNext.png'
                                    : 'assets/images/btnNextPressed.png'),
                                fit: BoxFit.contain),
                          ),
                          child: InkWell(
                  ...
                  msgShow.trim() == ''
                      ...
                      : alertMsg(context, msgShow, marge: 25),
                ],
              ),
            ),
          ),
