...
class _TheHomePageState extends State<TheHomePage> {
  ...
  initState() {
    ...
    if (firstOpen) player.play('audio/notif.mp3');
    firstOpen = false;
  ...
  loadHeroPic() async {
    String val;
    if (UserController.user.pictureName == null ||
        UserController.user.pictureName == '')
      val = await db.getPicture();
    else
      val = UserController.user.pictureName;
    setState(() {
      if (val != null && val != '')
        urlavatar = val;
      else
        urlavatar = 'assets/images/imageprofile.png';
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (fntTitle > 49) fntTitle = 49;
    var fntElem = H * 0.03; //w * 0.057;// W * 0.032;//H * 0.057
    if (fntElem > 22) fntElem = 22;
                  ...
                  GestureDetector(
                    onTap: () {
                      if (UserController.user.role != 'admin')
                        ...
                        Navigator.of(context).pushNamed('/leaderAdm');
                    },
                    child: Container(
                      height: hElem,
                      width: wElem,
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => BlocProvider(
                            create: (context) =>
                                TimerBloc(ticker: Ticker(), duration: 25 * 60),
                            child: TimerScreen(),
                          ),
                        ),
                      );
          ...
          decoration: BoxDecoration(
            image: DecorationImage(
              image: AssetImage(loadPrs
                  ? 'assets/images/exitpressed.png'
                  : 'assets/images/exitnopressed.png'),
              fit: BoxFit.contain,
            ),
          ),
          child: InkWell(
